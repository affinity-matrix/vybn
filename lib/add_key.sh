...
_fetch_github_keys() {
    ...
    info "Fetching keys from github.com/${username}..."
    local response
    response="$(curl -sf --max-time 10 "https://github.com/${username}.keys")" || {
        error "Could not fetch keys from github.com/${username}.keys"
        error "Check the username and try again."
        exit 1
    }

    if [[ -z "$response" ]]; then
        error "No keys found for GitHub user '${username}'"
        exit 1
    fi
    ...
    if (( count == 0 && errors == 0 )); then
        warn "No valid keys found for GitHub user '${username}'"
    elif (( errors > 0 )); then
        warn "Processed ${count} key(s), ${errors} skipped due to errors"
    else
        success "Processed ${count} key(s) from github.com/${username}"
    fi
}
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --sshid)
                [[ -n "$mode" ]] && { error "Only one key source allowed"; exit 1; }
                mode="sshid"
                sshid_user="${2:-}"
            ...
            --github)
                ...
                mode="github"
                github_user="${2:-}"
                [[ -z "$github_user" ]] && { error "--github requires a username"; exit 1; }
                shift 2
                ;;
            --file)
                [[ -n "$mode" ]] && { error "Only one key source allowed"; exit 1; }
    ...
    case "$mode" in
        ...
        github) _fetch_github_keys "$github_user" ;;
...
Usage:
  ...
  vybn add-key --github <username>        Fetch keys from GitHub
...
Modes:
  ...
  --github <username>  Fetch public keys from https://github.com/<username>.keys.
