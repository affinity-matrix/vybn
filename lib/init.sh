...
_init_non_interactive() {
    ...
    if [[ "${VYBN_PROVIDER}" == "ssh" ]] && [[ -z "${VYBN_SSH_HOST:-}" ]]; then
        error "VYBN_SSH_HOST is required for SSH provider in non-interactive mode"
        exit 1
    fi

    if [[ "${VYBN_NETWORK}" == "tailscale" ]] && [[ -z "${VYBN_TAILSCALE_AUTHKEY:-}" ]]; then
        error "VYBN_TAILSCALE_AUTHKEY is required for Tailscale in non-interactive mode"
        exit 1
    fi

    # Write config from current environment
    local tmpfile="${rcfile}.tmp.$$"

    {
        echo "# vybn configuration — generated by 'vybn init --non-interactive'"
        echo ""
        echo "VYBN_PROVIDER=\"${VYBN_PROVIDER}\""
        echo "VYBN_NETWORK=\"${VYBN_NETWORK}\""
        echo ""

        if [[ "${VYBN_PROVIDER}" == "ssh" ]]; then
            echo "VYBN_SSH_HOST=\"${VYBN_SSH_HOST}\""
            echo "VYBN_SSH_USER=\"${VYBN_SSH_USER}\""
            [[ -n "${VYBN_SSH_KEY}" ]] && echo "VYBN_SSH_KEY=\"${VYBN_SSH_KEY}\""
            [[ "${VYBN_SSH_PORT}" != "22" ]] && echo "VYBN_SSH_PORT=\"${VYBN_SSH_PORT}\""
        else
            [[ -n "${VYBN_PROJECT}" ]] && echo "VYBN_PROJECT=\"${VYBN_PROJECT}\""
            echo "VYBN_ZONE=\"${VYBN_ZONE}\""
            echo "VYBN_MACHINE_TYPE=\"${VYBN_MACHINE_TYPE}\""
            echo "VYBN_DISK_SIZE=\"${VYBN_DISK_SIZE}\""
        fi

        echo ""
        [[ -n "${VYBN_VM_NAME}" ]] && echo "VYBN_VM_NAME=\"${VYBN_VM_NAME}\""

        if [[ "${VYBN_NETWORK}" == "tailscale" ]]; then
            echo "VYBN_TAILSCALE_AUTHKEY=\"${VYBN_TAILSCALE_AUTHKEY}\""
            [[ -n "${VYBN_TAILSCALE_HOSTNAME}" ]] && echo "VYBN_TAILSCALE_HOSTNAME=\"${VYBN_TAILSCALE_HOSTNAME}\""
            [[ -n "${VYBN_TAILSCALE_TAGS}" ]] && echo "VYBN_TAILSCALE_TAGS=\"${VYBN_TAILSCALE_TAGS}\""
        fi

        echo ""
        echo "VYBN_TOOLCHAINS=\"${VYBN_TOOLCHAINS}\""
        [[ -n "${VYBN_APT_PACKAGES}" ]] && echo "VYBN_APT_PACKAGES=\"${VYBN_APT_PACKAGES}\""
        [[ -n "${VYBN_NPM_PACKAGES}" ]] && echo "VYBN_NPM_PACKAGES=\"${VYBN_NPM_PACKAGES}\""
        [[ -n "${VYBN_SETUP_SCRIPT}" ]] && echo "VYBN_SETUP_SCRIPT=\"${VYBN_SETUP_SCRIPT}\""
    } > "$tmpfile"
...
main() {
    ...
    # Non-interactive mode: write config from current env vars
    ...
    if ! [[ -t 0 ]]; then
        ...
        error "Use --non-interactive to generate config from environment variables."
        exit 1
    fi
...
Options:
  ...
  --non-interactive    Generate config from environment variables (for CI/automation)
